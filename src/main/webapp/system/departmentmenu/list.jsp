...
<section>
<config:config key="department.delete" info="department"/>
...
<div class="DivGlobal clearfix">
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${department == 'logical' && menu == 'logical'}">
		...
		<div class="item-title-sub">
			<input type="hidden" id="query_departmentid" name="query_departmentid" value="<c:out value='${query_departmentid}'/>"/>
			<input type="hidden" id="query_menuid" name="query_menuid" value="<c:out value='${query_menuid}'/>"/>
			<span>部门名称：<input type="text" id="query_department" name="query_department" value="<c:out value='${query_department}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w1"><c:choose><c:when test="${data.departmentname != null}"><c:out value="${data.departmentname}"/></c:when><c:otherwise><c:out value="${data.department}"/></c:otherwise></c:choose></span></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w1"><c:choose><c:when test="${data.departmentname != null}"><c:out value="${data.departmentname}"/></c:when><c:otherwise><c:out value="${data.department}"/></c:otherwise></c:choose></span></span></td>
