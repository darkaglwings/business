...
<script type="text/javascript" src="../../js/system/post/list.js"></script>
</head>
<body>
<%@ include file="../../frame/head.jsp"%>
<section>
<config:config key="post.delete" info="delete"/>
<config:config key="menu.related" info="menu"/>
<config:config key="user.related" info="user"/>
<config:config key="related.type.post" info="pattern"/>
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${menu == 'all' || fn:contains(menu, 'post')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'menu'; menu('../postmenu/load.jspx', '../menu/load.jspx', 'content', 'menu');" class="itembtn16">菜单分配</a></span>
			</c:if>
			<c:if test="${user == 'all' || fn:contains(user, 'department')}">
				<c:if test="${pattern == 'single'}">
					<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'user'; user('../user/post.jspx', '../department/load.jspx', '../user/load.jspx', 'content', true, 'department');" class="itembtn19">人员分配</a></span>
				</c:if>
				<c:if test="${pattern == 'multi'}">
					<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'user'; user('../userpost/load.jspx', '../department/load.jspx', '../user/load.jspx', 'content', true, 'post');" class="itembtn19">人员分配</a></span>
				</c:if>
			</c:if>
		...
		<div class="item-title-sub">
			<span>职务名称：<input type="text" id="query_title" name="query_title" value="<c:out value='${query_title}'/>"/></span>
			<span>上级职务：<input type="text" id="query_parentname" name="query_parentname" value="<c:out value='${query_parentname}'/>"/></span>
