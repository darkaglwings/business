...
<meta charset="utf-8" />
<title>职务信息管理</title>
<script type="text/javascript" src="../../js/system/post/detail.js"></script>
</head>
<body>
...
<div class="DivGlobal clearfix">
	...
	<div class="Content nontit">
		...
		<h3 class="mode-tit">职务信息详情</h3>
		<div class="tab-area">
			<div style="display:none">
				<input type="hidden" id="id" name="id" value="<c:out value='${post.id}'/>"/>
				<input type="hidden" id="flag" name="flag" value="<c:out value='${post.flag}'/>"/>
				<input type="hidden" id="fullname" name="fullname" value="<c:out value='${post.fullname}'/>"/>
				<input type="hidden" id="rank" name="rank" value="<c:out value='${post.rank}'/>"/>
				<input type="text" id="parentid" name="parentid" value="<c:out value='${post.parentid}'/>" style="width:90%"/>
			</div>
			<table cellpadding="0" cellspacing="0" class="tab-style2 tab-detail">
				<tr>
					<td class="item-name">职务名称：</td>
					<td><input type="text" id="title" name="title" value="<c:out value='${post.title}'/>" style="width:99%" required/></td>
					<td class="item-name">上级职务：</td>
					<td>
						<input type="text" id="parentname" name="parentname" value="<c:out value='${post.parentname}'/>" style="width:90%" readonly/>
				...
				<tr>
					...
					<td colspan="3"><textarea id="remark" name="remark" style="width:99%" rows="3"><c:out value='${post.remark}'/></textarea></td>
				</tr>
			</table>
		</div>
	</div>
