...
<config:config key="role.delete" info="role"/>
...
<div class="DivGlobal clearfix">
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${role == 'logical' && menu == 'logical'}">
		...
		<div class="item-title-sub">
			<input type="hidden" id="query_roleid" name="query_roleid" value="<c:out value='${query_roleid}'/>"/>
			<input type="hidden" id="query_menuid" name="query_menuid" value="<c:out value='${query_menuid}'/>"/>
			<span>角色名称：<input type="text" id="query_role" name="query_role" value="<c:out value='${query_role}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w1"><c:choose><c:when test="${data.rolename != null}"><c:out value="${data.rolename}"/></c:when><c:otherwise><c:out value="${data.role}"/></c:otherwise></c:choose></span></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w1"><c:choose><c:when test="${data.rolename != null}"><c:out value="${data.rolename}"/></c:when><c:otherwise><c:out value="${data.role}"/></c:otherwise></c:choose></span></span></td>
