...
<section>
<config:config key="dictionary.delete" info="delete"/>
...
<div class="DivGlobal clearfix">
	...
	<div class="Content">
		...
		<div class="item-title-sub">
			<span>上级字典：<input type="text" id="query_parentname" name="query_parentname" value="<c:out value='${query_parentname}'/>"/></span>
			<span>字典类型：<input type="text" id="query_sort" name="query_sort" value="<c:out value='${query_sort}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w1"><c:choose><c:when test="${data.parentname != null}"><c:out value="${data.parentname}"/></c:when><c:otherwise><c:choose><c:when test="${data.parentid == 0}"><c:out value="${''}"/></c:when><c:otherwise><c:out value="${data.parentid}"/></c:otherwise></c:choose></c:otherwise></c:choose></span></td>
							<td><span class="tab-w2"><c:out value="${data.sort}"/></span></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w1"><c:choose><c:when test="${data.parentname != null}"><c:out value="${data.parentname}"/></c:when><c:otherwise><c:choose><c:when test="${data.parentid == 0}"><c:out value="${''}"/></c:when><c:otherwise><c:out value="${data.parentid}"/></c:otherwise></c:choose></c:otherwise></c:choose></span></td>
							<td class="tds01"><span class="tab-w2"><c:out value="${data.sort}"/></span></td>
							<td class="tds01"><c:out value="${data.code}"/></td>
							<td class="tds01"><c:out value="${data.meaning}"/></td>
