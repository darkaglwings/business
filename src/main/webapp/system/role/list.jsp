...
<section>
<config:config key="role.delete" info="delete"/>
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${menu == 'all' || fn:contains(menu, 'role')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'menu'; menu('../rolemenu/load.jspx', '../menu/load.jspx', 'content', 'menu');" class="itembtn16">菜单分配</a></span>
			</c:if>
			<c:if test="${user == 'all' || fn:contains(user, 'role')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'user'; user('../userrole/load.jspx', '../department/load.jspx', '../user/load.jspx', 'content', true, 'role');" class="itembtn19">人员分配</a></span>
			</c:if>
		...
		<div class="item-title-sub">
			<span>角色名称：<input type="text" id="query_title" name="query_title" value="<c:out value='${query_title}'/>"/></span>
			<span>角色等级：<input type="text" id="query_rank" name="query_rank" value="<c:out value='${query_rank}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w2"><c:out value="${data.rank}"/></span></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w2"><c:out value="${data.rank}"/></span></td>
