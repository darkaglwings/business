...
<script type="text/javascript" src="../../js/system/menu/list.js"></script>
</head>
<body>
<%@ include file="../../frame/head.jsp"%>
<section>
<config:config key="menu.delete" info="delete"/>
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${menu == 'all' || fn:contains(menu, 'department')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'department'; menu('../departmentmenu/load.jspx', '../department/load.jspx', 'content', 'department');" class="itembtn33">部门分配</a></span>
			</c:if>
			<c:if test="${menu == 'all' || fn:contains(menu, 'post')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'post'; menu('../postmenu/load.jspx', '../post/load.jspx', 'content', 'post');" class="itembtn32">职务分配</a></span>
			</c:if>
			<c:if test="${menu == 'all' || fn:contains(menu, 'role')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'role'; role('../rolemenu/load.jspx', '../role/load.jspx', 'role', 'menu');" class="itembtn18">角色分配</a></span>
			</c:if>
			<c:if test="${menu == 'all' || fn:contains(menu, 'user')}">
				<span style="display:block"><a href="javascript:void(0);" onclick="javascript: selector = 'user'; user('../usermenu/load.jspx', '../department/load.jspx', '../user/load.jspx', 'content', true, 'menu');" class="itembtn19">人员分配</a></span>
			</c:if>
		...
		<div class="item-title-sub">
			<span>菜单名称：<input type="text" id="query_title" name="query_title" value="<c:out value='${query_title}'/>"/></span>
			<span>所属系统：<input type="text" id="query_system" name="query_system" value="<c:out value='${query_system}'/>"/></span>
			<span>上级菜单：<input type="text" id="query_parentname" name="query_parentname" value="<c:out value='${query_parentname}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w2"><c:out value="${data.system}"/></span></td>
							<td><c:out value="${data.parentname}"/></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w2"><c:out value="${data.system}"/></span></td>
							<td class="tds01"><c:out value="${data.parentname}"/></td>
