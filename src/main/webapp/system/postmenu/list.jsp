...
<config:config key="post.delete" info="post"/>
...
<div class="DivGlobal clearfix">
	...
	<div class="Content">
		<div class="item-title">
			...
			<c:if test="${post == 'logical' && menu == 'logical'}">
		...
		<div class="item-title-sub">
			<input type="hidden" id="query_postid" name="query_postid" value="<c:out value='${query_postid}'/>"/>
			<input type="hidden" id="query_menuid" name="query_menuid" value="<c:out value='${query_menuid}'/>"/>
			<span>职务名称：<input type="text" id="query_post" name="query_post" value="<c:out value='${query_post}'/>"/></span>
				...
				<c:choose>
					<c:when test="${i % 2 == 0}">
						<tr>
							...
							<td><span class="tab-w1"><c:choose><c:when test="${data.postname != null}"><c:out value="${data.postname}"/></c:when><c:otherwise><c:out value="${data.post}"/></c:otherwise></c:choose></span></td>
					...
					<c:otherwise>
						<tr>
							...
							<td class="tds01"><span class="tab-w1"><c:choose><c:when test="${data.postname != null}"><c:out value="${data.postname}"/></c:when><c:otherwise><c:out value="${data.post}"/></c:otherwise></c:choose></span></span></td>
